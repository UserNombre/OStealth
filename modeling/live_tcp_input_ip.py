...
ATTACKER_IP = sys.argv[3] if len(sys.argv) > 3 else None

OUTDIR = "./processed_traffic"
TS = time.strftime("%Y%m%d_%H%M%S")
CSV_FILE = f"{OUTDIR}/live_tcp_{TS}.csv"

if not IFACE or not MAX_PACKETS or not ATTACKER_IP:
    print(f"Uso: sudo {sys.argv[0]} <interfaz> <num_paquetes> <ip_atacante>")
    sys.exit(1)
