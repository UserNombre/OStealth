...
# =====================================================================
# GENERADOR: SOLO Scapy (Tráfico Realista, SIN Nmap)
# =====================================================================
# Genera:
#   - Tráfico TCP REALISTA con handshakes completos (Scapy)
#   - SIN escaneos nmap (0 fingerprinting activo)
#
# Uso:
#   sudo ./generate_traffic_realistic_scapy_no_nmap.sh <target> <interfaz> <duracion_seg>
#
# Ejemplo:
#   sudo ./generate_traffic_realistic_scapy_no_nmap.sh 192.168.1.1 eth0 600
# =====================================================================
...
if [ "$1" == "-h" ] || [ "$1" == "--help" ] || [ -z "$TARGET" ] || [ -z "$IFACE" ] || [ -z "$DURATION" ]; then
    cat << 'EOF'
╔══════════════════════════════════════════════════════════════════════╗
║  GENERADOR: SOLO Scapy (SIN Nmap)                                    ║
╚══════════════════════════════════════════════════════════════════════╝

DESCRIPCIÓN:
  Genera tráfico TCP REALISTA con handshakes completos usando Scapy.
  NO lanza ningún escaneo nmap (cero fingerprinting activo).

USO:
  sudo ./generate_traffic_realistic_scapy_no_nmap.sh <target> <interfaz> <duracion>
...
EJEMPLOS:
  sudo ./generate_traffic_realistic_scapy_no_nmap.sh 192.168.1.1 eth0 600
  sudo ./generate_traffic_realistic_scapy_no_nmap.sh 10.0.0.1 wlan0 1800
...
╔══════════════════════════════════════════════════════════════════════╗
║        GENERADOR: SOLO Scapy (Realista, SIN Nmap)                   ║
╚══════════════════════════════════════════════════════════════════════╝
  ...
  🔍 Fingerprinting:
     • Ninguno (SIN nmap)
     • Si ves type_f=1 en el CSV, algo raro pasa 😉
...
print()
print("=" * 70)
print("RESUMEN TRÁFICO SCAPY (SIN NMAP)")
print("=" * 70)
...
echo "" | tee -a "$LOG_FILE"
echo "╔══════════════════════════════════════════════════════════════════════╗" | tee -a "$LOG_FILE"
echo "║  RESUMEN FINAL (SIN NMAP)                                            ║" | tee -a "$LOG_FILE"
echo "╚══════════════════════════════════════════════════════════════════════╝" | tee -a "$LOG_FILE"
echo "  Tráfico Scapy generado: Ver detalles arriba" | tee -a "$LOG_FILE"
echo "  Escaneos nmap ejecutados: 0" | tee -a "$LOG_FILE"
